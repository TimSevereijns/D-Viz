...
namespace Settings
{
    ...
    class PreferencesMap
    {
      public:
         ...
         */
        template <typename DataType> void Emplace(std::wstring name, DataType&& data);
        ...
        RequestedType
        GetValueOrDefault(std::wstring_view query, const RequestedType& defaultValue) const;
...
namespace
{
     ...
     */
    template <typename, typename = void> struct IsSupportedType : std::false_type {
    };
...
namespace Settings
{
    template <typename DataType> void PreferencesMap::Emplace(std::wstring name, DataType&& data)
    {
        static_assert(
    ...
    RequestedType PreferencesMap::GetValueOrDefault(
        std::wstring_view query, const RequestedType& defaultValue) const
    {
        static_assert(
