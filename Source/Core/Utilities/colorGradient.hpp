    ...
    void AddColorPoint(float red, float green, float blue, float value)
    {
        for (auto i = 0u; i < m_colorPoints.size(); i++) {
            if (value < m_colorPoints[i].normalizedValue) {
                m_colorPoints.insert(
                    std::begin(m_colorPoints) + i, ColorPoint{ red, green, blue, value });
    ...
    QVector3D GetColorAtValue(const float value)
    {
        ...
        for (size_t i = 0; i < m_colorPoints.size(); i++) {
            ...
            if (value < currentColor.normalizedValue) {
                ...
                const auto fractBetween =
                    (delta == 0.0f) ? 0.0f : (value - currentColor.normalizedValue) / delta;
        ...
        return { m_colorPoints.back().red, m_colorPoints.back().green, m_colorPoints.back().blue };
    }
